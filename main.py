...
# После первой реликвии может случайно выпасть любая реликвия,
# включая Леди Баг.
...
async def save_user(user: dict) -> None:
    # Поле _id нельзя передавать в $set.
    # Иначе MongoDB выдаёт ошибку обновления документа.
    data = {
        key: value
...
async def profile(
    ...
    if user.get("ability_used") and user.get("last_use"):
        ...
        if elapsed < ABILITY_COOLDOWN:
            ...
            timer_line = "✨ Способность готова к активации!"
    else:
        timer_line = "✨ Способность готова к активации!"
...
async def power(
    ...
    # Если /power написали ответом на сообщение игрока,
    # передаём обработку выбора цели в handle_reply.
    if (
    ...
    # Первая реликвия всегда Леди Баг.
    # После этого реликвии выдаются случайно,
    # включая Леди Баг.
    if not user.get("kwami"):
        if user.get("is_new"):
            kwami = FIRST_RELIC
    ...
    # Проверяем таймер после применения способности.
    if user.get("ability_used") and user.get("last_use"):
        last_use = to_naive_utc(user["last_use"])
        elapsed = (now - last_use).total_seconds()
...
# =========================================================
# ОБРАБОТКА ОТВЕТОВ НА СООБЩЕНИЯ ИГРОКОВ
# =========================================================
...
async def main():
    ...
    # Обычный текстовый ответ на сообщение игрока.
    # Команда /power обрабатывается внутри функции power().
    application.add_handler(
        MessageHandler(
