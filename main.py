...
async def start_registration(
    ...
    # Проверяем, что игрок не в раунде
    if player["in_round"]:
        await query.answer(
            "❌ Вы уже в раунде!",
            show_alert=True
...
async def resources(
    ...
    if player["forest_started_at"] is not None:
        ...
        # 50/50 на древесину или металл
        if random.choice([True, False]):

            logs_gained = random.randint(3, 7)
            food_gained = 2

            await update_player(
                user_id,

                logs=player["logs"] + logs_gained,
                food=player["food"] + food_gained,
            ...
            await query.answer(
                f"🍃 Добыча ресурсов окончена! Получено: 🪵 {logs_gained} брёвен, 🍗 2 еда",
                show_alert=True
            )

        else:

            metal_gained = random.randint(3, 7)
            food_gained = 2

            await update_player(
                user_id,

                metal=player["metal"] + metal_gained,
                food=player["food"] + food_gained,
            ...
            await query.answer(
                f"🍃 Добыча ресурсов окончена! Получено: 🔩 {metal_gained} металла, 🍗 2 еда",
                show_alert=True
            )
