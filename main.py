...
async def profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if kwami:
        ...
        ability_text = f"{emoji} Способность: {kwami}"
    else:
        emoji = "🐢"
...
async def power(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если способность уже использована
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data in ["daily_bonus", "reset_timer", "change_relic"]:
        await query.message.reply_text("⏳ Функция находится на стадии разработки!")
